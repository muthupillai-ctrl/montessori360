...
export async function generateProgressCard(
  ...
  const doc = new PDFDocument({
    ...
    info: { Title: `Progress Card — ${student.full_name}`, Author: 'Taji One' },
  });
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc.fillColor(WHITE).font(F.regular).fontSize(8)
       .text(
         `Generated by Taji One  ·  Confidential  ·  Page ${i + 1} of ${range.count}`,
         L, pageH - 22, { width: W }
       );
  }
...
function renderCover(ctx: RenderContext) {
  ...
  if (template.logo_url) {
    ...
    doc.fillColor(WHITE).font(F.bold).fontSize(20).text(student.class_name ?? 'Taji One', L + 70, 20);
    doc.font(F.regular).fontSize(10).text('Child Progress Report', L + 70, 46);
    doc.font(F.regular).fontSize(10).text(term, L + 70, 62);
  } else {
    doc.fillColor(WHITE).font(F.bold).fontSize(22).text('Taji One', L, 18);
